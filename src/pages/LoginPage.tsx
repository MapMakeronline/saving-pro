...
export default function LoginPage() {
  ...
  const { login, loginWithGoogle, loginWithGithub, loginWithFacebook, loginWithMicrosoft, isLoading, error, clearError } = useAuthStore()
      ...
      >
        <div className={`rounded-[28px] shadow-2xl ${th.shadow} p-8`} style={{ backgroundColor: th.bgCard }}>
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className={`text-2xl font-black ${th.text} tracking-tight`}>{t('auth.loginTitle')}</h1>
            <p className={`${th.textSub} text-sm mt-1.5`}>{t('auth.loginSubtitle')}</p>
          </div>
          ...
          <p className={`text-center mt-5 ${th.link} text-sm`}>
            {t('auth.noAccount')}{' '}
            <Link to="/register" className={`${th.teal} hover:opacity-80 font-semibold transition-opacity cursor-pointer`}>
